...
# Predict probabilities on test set using lambda.min and lambda.1se
prob_min <- predict(
...
# Set custom threshold to increase recall
threshold <- 0.4

# Apply threshold manually to get class predictions
pred_class_min <- ifelse(prob_min >= threshold, 1, 0)
pred_class_1se <- ifelse(prob_1se >= threshold, 1, 0)

# Create confusion matrices
conf_mat_min <- table(
  actual = as.numeric(test_y),
  predicted = pred_class_min
)

conf_mat_1se <- table(
  actual = as.numeric(test_y),
  predicted = pred_class_1se
)
...
for (i in seq_along(thresholds)) {
  ...
  pred_class <- ifelse(prob_min >= t, 1, 0)
  conf_mat <- table(
    actual = as.numeric(test_y),
...
plot(
  ...
  xlab = "Threshold",
  ylab = "Metric Value",
  main = "Recall and Precision vs Threshold"
)
lines(
